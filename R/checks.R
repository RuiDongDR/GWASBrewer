...
check_G <- function(G, h2) {
  ...
  if (any(direct_h2 < 0)) {
    stop(paste0(
      "Supplied G is incompatible with supplied h2. You could try increasing the heritability of traits ",
...
check_01 <- function(x, name) {
  if (any(x < 0) | any(x > 1)) {
    stop(paste0(
      "Expected ", name, " to be between 0 and 1. Check elements",
...
check_R_LD <- function(R_LD, return = c("eigen", "matrix", "sqrt", "l")) {
  ...
  if (return == "eigen") {
    if (all(cl == "matrix")) {
      ...
      ii <- which(cl == "matrix")
      R_LD[ii] <- lapply(R_LD[ii], function(x) {
        eigen(x)
      })
    }
    ...
    if (all(cl == "eigen")) {
      ...
      R_LD <- lapply(R_LD, function(m) {
        x <- eigen(m)
        x$values[x$values < 0] <- 0 ##### edit by Rui on 20240624
        tcrossprod(x$vectors, diag(sqrt(x$values)))
      })
    } else {
      ie <- which(cl == "eigen")
    ...
    if (all(cl == "eigen")) {
      ...
      ie <- which(cl == "eigen")
      l[ie] <- sapply(R_LD[ie], function(x) {
        length(x$values)
      })
      im <- which(cl == "matrix")
      l[im] <- sapply(R_LD[im], function(m) {
        nrow(m)
      })
    }
    return(l)
